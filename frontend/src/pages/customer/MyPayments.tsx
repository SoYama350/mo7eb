...
export function MyPayments() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-sm font-bold text-brand-700">الدفع الآمن</p><h1 className="section-title mb-1">مدفوعاتي</h1><p className="text-sm text-slate-500">ارفع صورة التحويل وتابع قرار المراجعة من هنا.</p></div><Link to="/subscriptions" className="btn btn-outline">عرض الاشتراكات</Link></div>
     ...
     <section className="card p-5 sm:p-6"><div><h2 className="text-lg font-black text-night">بيانات التحويل</h2><p className="mt-1 text-xs leading-6 text-slate-500">استخدم الرقم أو الحساب الظاهر هنا فقط عند التحويل، ثم ارفع صورة الإيصال.</p></div>{methods.length === 0 ? <p className="mt-4 rounded-xl bg-amber-50 p-4 text-sm font-bold text-amber-800">الإدارة لم تضف وسيلة دفع متاحة حتى الآن.</p> : <div className="mt-4 grid gap-3 md:grid-cols-2">{methods.map((method) => <article key={method.id} className="rounded-2xl border border-brand-100 bg-brand-50/60 p-4"><p className="font-black text-night">{method.name}</p>{method.accountIdentifier ? <p className="mt-2 text-xl font-black tracking-wide text-brand-800" dir="ltr">{method.accountIdentifier}</p> : <p className="mt-2 text-sm font-bold text-amber-700">رقم الحساب غير مضاف من الإدارة</p>}{method.instructions && <p className="mt-2 text-xs leading-6 text-slate-600">{method.instructions}</p>}</article>)}</div>}</section>
