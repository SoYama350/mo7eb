...
const MSG_EXP = 'باقة 40 GB هتنتهي بعد 20 يوم. جدد قبل معاد الانتهاء.';
const T_EXPIRED = 'الباقة خلصت';
const MSG_EXPIRED = 'باقتك خلصت. جدد دلوقتي لتكمل نت.';
...
export async function runExpiryJob(trigger: string = 'cron') {
  const now = new Date();
  const soonThreshold = new Date(now.getTime() + 7 * 24 * 3600 * 1000);
 ...
 for (const sub of active) {
    ...
    if (daysLeft <= sub.package.reminderDays && daysLeft > 0) {
      ...
      await prisma.notification.create({ data: { userId: sub.userId, type: 'subscription.expiring', title: T_EXP, message: MSG_EXP } });
      results.expiringSoon++;
    } else if (daysLeft <= 0) {
      await prisma.subscription.update({ where: { id: sub.id }, data: { status: "EXPIRED" } } );
      await prisma.notification.create({ data: { userId: sub.userId, type: 'subscription.expired', title: T_EXPIRED, message: MSG_EXPIRED } });
      results.expired++;
    }
  }
